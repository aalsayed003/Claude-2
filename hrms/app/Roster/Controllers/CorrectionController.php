...
class CorrectionController extends Controller
{
    ...
    {
        if (legacy_mode()) {
            $rows = (new \App\Roster\Repositories\RosterRepository($this->db))->forEmployeeRange($empId, $from, $to);
        } else {
            $rows = [];
            foreach ($this->db->all(
    ...
    public function save(): void
    {
        ...
        if (legacy_mode()) {
            ...
            try {
                (new \App\Roster\Repositories\CorrectionRepository($this->db))->create([
                    'employee_id' => $empId,
                    'work_date'   => $date,
