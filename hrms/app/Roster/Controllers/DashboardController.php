...
class DashboardController extends Controller
{
    ...
    private function legacyCounts(string $period, string $start, string $end, string $today): array
    {
        ...
        try {
            $todayCounts = \App\Roster\Services\DashboardMetrics::todayCounts($this->db, $today);
        } catch (\Throwable $e) {
            $todayCounts = ['absent' => 0, 'odd_punch' => 0, 'late' => 0, 'early' => 0, 'day_off' => 0];
        }

        return [
            'schedules' => $safe(fn() => (new \App\Roster\Repositories\ScheduleRequestRepository($this->db))->pendingCount()),
            'corrections' => $safe(fn() => (new \App\Roster\Repositories\CorrectionRepository($this->db))->pendingCount($start, $end)),
    ...
    public function detail(): void
    {
        ...
        $rows = legacy_mode()
            ? \App\Roster\Services\DashboardMetrics::todayDetails($this->db, $date, $metric)
            : [];

        $this->view('dashboard/detail', [
            'title'  => \App\Roster\Services\DashboardMetrics::metricLabel($metric) . ' — ' . date('d M Y', strtotime($date)),
            'metric' => $metric,
            'label'  => \App\Roster\Services\DashboardMetrics::metricLabel($metric),
            'date'   => $date,
            'period' => $period,
            'rows'   => $rows,
        ]);
