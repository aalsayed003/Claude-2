...
class EmployeeController extends Controller
{
    public function index(): void
    {
        ...
        $q = $this->input('q', '');
        if (legacy_mode()) {
            $emps = (new \App\Roster\Repositories\EmployeeRepository($this->db))->search($q);
        } else {
            $params = [];
            $where = '1=1';
            if ($q !== '') {
    ...
    public function edit(): void
    {
        ...
        $emp = legacy_mode()
            ? (new \App\Roster\Repositories\EmployeeRepository($this->db))->find($id)
    ...
    {
        if (legacy_mode()) {
            return [(new \App\Roster\Repositories\DepartmentRepository($this->db))->all(), []];
        }
        return [
