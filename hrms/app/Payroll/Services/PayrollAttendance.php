...
class PayrollAttendance
{
    ...
    public function load(string $from, string $to): void
    {
        ...
        // Prefer the RAW punch feed paired against the roster (same schedule-aware
        // pairing as View Attendance — correct for split-duty and overnight). Fall
        // back to the pre-paired Atten_MMYYYY tables only when the raw feed isn't
        // reachable.
        if (!$this->loadAttendanceFromPunches($from, $to)) {
    ...
    /**
     * Bulk-load the raw biometric punches for the cycle and pair each employee's
     * day against their rostered shift with the roster module's PunchPairer, so
     * payroll sees exactly what View Attendance sees. One punch query for the
     * whole period (day-bounded), then paired in memory. Returns false when the
     * raw punch table isn't reachable so the caller falls back to Atten_.
     */
